...
static void hybrid_run()
{
    ...
    if (!failsafe.radio) {
		...
		// get pilot's desired throttle (for manual "throttle assist" mode)
	...
	if (ap.land_complete) {
		...
		switch (hybrid_mode_roll){
			...
			case 1: { 
						...
						target_roll = loiter_man_mix*(float)loiter_roll+(1.0f-loiter_man_mix)*(float)(brake_roll+wind_offset_roll); break;
					}
		...
		switch (hybrid_mode_pitch){
			case 1: { 
						//Loiter-Manual mix at loiter exit
						target_pitch = loiter_man_mix*(float)loiter_pitch+(1.0f-loiter_man_mix)*(float)(brake_pitch+wind_offset_pitch); break;
					}
	...
	switch (throttle_mode)
	{
		...
		if ((hybrid_mode_pitch==3) && (hybrid_mode_roll==3)) {
			...
			pos_control.set_alt_target_from_climb_rate(target_climb_rate, G_Dt);
			pos_control.update_z_controller();
		} else { // Manual throttle when repositioning: JASON's Throttle assist
			// Grab inertial velocity
			Vector3f vel = inertial_nav.get_velocity();
			float myGain = 1.0 - ((float)abs(pilot_throttle_scaled - g.throttle_cruise) / 200.0);
			myGain = max(myGain, 0) * -2; // 2 is the gain
			attitude_control.set_throttle_out(pilot_throttle_scaled + (vel.z * myGain), true);
		}
#endif
